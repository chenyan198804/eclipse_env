...
def gen_secret_key():
	global yb, zz, sk, SK_d, SK_ai, SK_ar, SK_ei, SK_er, SK_pi, SK_pr
...
while True:
    ...
    if ike_auth_r is None or ike_auth_r[Ether].src == src_mac:
       continue  
        ...
        def dissert_ikev2_decrypted_payload(s):
            global ikev2_payload_decrypted
            position    = 0
            payload_type= []
            payload     = []
            payload_type.append(ikev2_payload_decrypted.next_payload)

            while position < len(str(s)):
                payload_len = int(toHex(s[position + 2:position + 4]), 16)
                payload.append(s[position:position+payload_len])
                t = int(toHex(s[position:position+1]), 16)
        ...
        dissert_ikev2_decrypted_payload(decrypted_data)
        # ikev2.show2()
...
def gen_ipsc_secret_key(ike_nonce_i, ike_nonce_r):
	# global SK_ei_ipsec, SK_ai_ipsec, SK_er_ipsec, SK_ar_ipsec, ipsec_spi_i, ipsec_spi_r
	prf_plus_seed = ike_nonce_i + ike_nonce_r
	...
	print conf.setkey
	return (SK_ei_ipsec, SK_ai_ipsec, SK_er_ipsec, SK_ar_ipsec)


'''keys log'''
def save_keys_log(ike_spi_i, ike_spi_r, SK_ei, SK_er, SK_ai, SK_ar, SK_ei_ipsec, SK_ai_ipsec, SK_er_ipsec, SK_ar_ipsec):
...
def del_ike_sa(ike_spi_i, ike_spi_r, init_vector, SK_ai, SK_ei):
	global ike_message_id
...
def gen_create_child_sa_message(ipsec_spi_i, SK_ei, SK_ai):
	global ike_message_id
	...
	ikev2_payload_nonce = IKEv2_payload_Nonce()
	ikev2_payload_nonce.next_payload = 0x2c     #no more payload
	ike_nonce_i = ''.join(RandString(32))
	print "line %d: ike_nonce_i is \n%s" % (lineno(), toHex(ike_nonce_i))
	ikev2_payload_nonce.load = ike_nonce_i
	...
	return (ipsec_spi_i, ike_nonce_i)

(SK_ei_ipsec, SK_ai_ipsec, SK_er_ipsec, SK_ar_ipsec) = gen_ipsc_secret_key(ike_nonce_i, ike_nonce_r)
save_keys_log(ike_spi_i, ike_spi_r, SK_ei, SK_er, SK_ai, SK_ar, SK_ei_ipsec, SK_ai_ipsec, SK_er_ipsec, SK_ar_ipsec)
...
(ipsec_spi_i, ike_nonce_i) = gen_create_child_sa_message(ipsec_spi_i, SK_ei, SK_ai)

del_ipsec_sa(ike_spi_i, ike_spi_r, ipsec_spi_i_ori, SK_ai, SK_ei)
del_ipsec_sa(ike_spi_i, ike_spi_r, ipsec_spi_i, SK_ai, SK_ei)
del_ike_sa(ike_spi_i, ike_spi_r, init_vector, SK_ai, SK_ei)
