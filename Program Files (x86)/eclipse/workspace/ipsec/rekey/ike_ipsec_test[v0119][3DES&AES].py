...
##    - add AES_CBC encryption function
...
ENCRYPTION_MODE = "AES_CBC"        #"AES_CBC" or "3DES"
PRF_MODE        = "PRF_HMAC_SHA1"
INTEGRITY_MODE  = "HMAC_SHA1_96"
DH_GROUP_MODE   = "1024MODPgr"
ENCRYPTION_BLOCK_SIZE = {"3DES" : 8, "AES_CBC" : 16}
...
def gen_ikes_sa_init(ike_spi_i, ike_nonce_i, ya):
    ...
    if ENCRYPTION_MODE == "3DES":
        ...
        ikev2_payload_transform_encryption.key_length = "\x80\x0e\x00\x80"
...
if ENCRYPTION_MODE == "3DES":
    ...
    ipsec_payload_transform_encryption.key_length = "\x80\x0e\x00\x80"    #aes key attribute
...
ike_padding = ''
ike_pad_len = 0
if (len(plain_data) + 1)%ENCRYPTION_BLOCK_SIZE[ENCRYPTION_MODE] == 0:
    ike_padding = '\x00'
else:
    ike_pad_len = ((len(plain_data) + 1)/ENCRYPTION_BLOCK_SIZE[ENCRYPTION_MODE] + 1 )*ENCRYPTION_BLOCK_SIZE[ENCRYPTION_MODE] - (len(plain_data) + 1)
...
def payload_padding(packet_len):
    pad_len = 0
    if (packet_len + 2)%ENCRYPTION_BLOCK_SIZE[ENCRYPTION_MODE] == 0:            # the other 2 bytes ESP Trailer which continas "Pad Length" and "Next Header", refer the upper "Substructure of Payload Data"
        pad_len = 0
    else:
        pad_len = ((packet_len + 2)/ENCRYPTION_BLOCK_SIZE[ENCRYPTION_MODE] + 1 )*ENCRYPTION_BLOCK_SIZE[ENCRYPTION_MODE] - (packet_len + 2)
