...
####### History #############################################################
## wukun @31/12/2013
##    - add function save_ike_keys and save_ipsec_keys
##    - change the file location of keys, they are wrote to wireshark's files(ikev2_decryption_table and esp_sa)
## wukun @30/12/2013
##    - add function gen_ike_auth
##    - add global variabl psk
## wukun @27/12/2013
##    - add global variable ENCRYPTION_ALGOS
##    - modify the procedure of encryption
## wukun @25/12/2013  -- 27/12/2013
##    - add AES-CBC encryption function
...
'''
ENCRYPTION_MODE = "AES-CBC"        #"AES-CBC" or "3DES-CBC"
...
traffic_selector_local_ip  = ["10.69.196.100", "10.69.196.101"]
...
PRF_MODE        = "PRF_HMAC_SHA1"
INTEGRITY_MODE  = "HMAC_SHA1_96"
DH_GROUP_MODE   = "1024MODPgr"
ENCRYPTION_ALGOS = {'AES-CBC' :      ['AES',     'CBC', 16,   [16,24,32]],
                    ...
                    'NULL' :         [None,      None,   0,    [-1]]}
ENCRYPTION_BLOCK_SIZE = ENCRYPTION_ALGOS[ENCRYPTION_MODE][2]        #the length of encryption's iv equals the size of block
...
def save_ike_keys(ike_spi_i, ike_spi_r, SK_ai, SK_ar, SK_ei, SK_er):
    keys_log_file_name = "C:\\Documents and Settings\\lteiot-tester\\Application Data\\Wireshark\\ikev2_decryption_table"
    keys_log_file = open(keys_log_file_name, "a")
    if ENCRYPTION_MODE == "3DES-CBC":
        ike_encrypt_string = "3DES [RFC2451]"
        ike_authentication_string = "HMAC_SHA1_96 [RFC2404]"
    elif ENCRYPTION_MODE == "AES-CBC":
        ike_encrypt_string = "AES-CBC-128 [RFC3602]"
        ike_authentication_string = "HMAC_SHA1_96 [RFC2404]"
...
def save_ipsec_keys(SK_ai_ipsec, SK_ar_ipsec, SK_ei_ipsec, SK_er_ipsec):
    keys_log_file_name = "C:\\Documents and Settings\\lteiot-tester\\Application Data\\Wireshark\esp_sa"
    keys_log_file = open(keys_log_file_name, "a")
    if ENCRYPTION_MODE == "3DES-CBC":
        ipsec_encrypt_string = "TripleDES-CBC [RFC2451]"
        ipsec_authentication_string = "HMAC-SHA-1-96 [RFC2404]"
    elif ENCRYPTION_MODE == "AES-CBC":
        ipsec_encrypt_string = "AES-CBC [RFC3602]"
        ipsec_authentication_string = "HMAC-SHA-1-96 [RFC2404]"
...
def gen_ikes_sa_init(ike_spi_i, ike_nonce_i, ya):
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        ikev2_payload_transform_encryption = IKEv2_payload_Transform()
        ikev2_payload_transform_encryption.next_payload   = 0x03
        ikev2_payload_transform_encryption.length         = 0x0c
        ikev2_payload_transform_encryption.transform_type = 0x01   #encryption
        ikev2_payload_transform_encryption.transform_id   = 0x000c     #AES-CBC
        ikev2_payload_transform_encryption.key_length     = 128
        print "line %d\n" % lineno()
        ikev2_payload_transform_encryption.show2()
...
def gen_ike_key(xa, yb, ike_nonce_i, ike_nonce_r, ike_spi_i, ike_spi_r):
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        total_key_len = 20 + 20*2 + 16*2 + 20*2 #total_key_len = 132
    n = '\x01'
    t = ''
    sk = ''
    hmac_key    = toStr(skeyseed)
    for x in range(1, total_key_len/20+1):
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        SK_ei   = sk[len(SK_ar+SK_ar+SK_d):16+len(SK_ar+SK_ar+SK_d)]
        SK_er   = sk[len(SK_ei+SK_ar+SK_ar+SK_d):16+len(SK_ei+SK_ar+SK_ar+SK_d)]
...
save_ike_keys(ike_spi_i, ike_spi_r, SK_ai, SK_ar, SK_ei, SK_er)
...
def gen_ike_auth(psk, SK_ai, SK_pi, ike_nonce_r, ike_spi_i, ike_spi_r, traffic_selector_local_ip, traffic_selector_remote_ip):
    ...
    # elif ENCRYPTION_MODE == "AES-CBC":
        # init_vector = ''.join(RandString(16))
    init_vector = ''.join(RandString(ENCRYPTION_BLOCK_SIZE))
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        ipsec_payload_transform_encryption.next_payload = 0x03      #next payload is transform
        ipsec_payload_transform_encryption.res  = 0x0C              #RESERVERD
        ipsec_payload_transform_encryption.length = 0x0C
        ipsec_payload_transform_encryption.transform_type = 0x01    #encryption
        ipsec_payload_transform_encryption.transform_id = 0x000C    #encr_aes_128
        ipsec_payload_transform_encryption.key_length = 128    #aes key attribute
    ...
    ike_padding = ''
    ike_pad_len = 0
    if (len(plain_data) + 1)%ENCRYPTION_BLOCK_SIZE == 0:
        ike_padding = '\x00'
    else:
        ike_pad_len = ((len(plain_data) + 1)/ENCRYPTION_BLOCK_SIZE + 1 )*ENCRYPTION_BLOCK_SIZE - (len(plain_data) + 1)
    ...
    # elif ENCRYPTION_MODE == "AES-CBC":
        # aes_cbc_encrypt = AES.new(SK_ei, AES.MODE_CBC, init_vector)
        # encrypted_data = aes_cbc_encrypt.encrypt(plain_data)
    try:
        cipher = eval(ENCRYPTION_ALGOS[ENCRYPTION_MODE][0])
    except NameError:
        raise Exception('module <' + cipher + '> not available')
    enc=cipher.new(SK_ei, cipher.MODE_CBC, IV=init_vector)
    encrypted_data = enc.encrypt(plain_data)
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        ikev2_payload_encrypted = IKEv2_payload_Encrypted_AES()

    #IKEv2_payload_Encrypted.encrypt_mode = ENCRYPTION_MODE
    #ikev2_payload_encrypted = IKEv2_payload_Encrypted()
    ...
    while True:
        ...
        # elif ike_auth_r[UDP].sport == 500 and ike_auth_r.init_SPI == ike_spi_i:
            ...
            if ENCRYPTION_MODE == "3DES-CBC":
                ...
                #the init_vertor of AES_CBC is 16Bytes. So divide the 8bytes from field "load" to field "iv"
                init_vector_r   = ike_auth_r[IKEv2_payload_Encrypted].iv + ike_auth_r[IKEv2_payload_Encrypted].load[:8]     
                aes_cbc_decrypt = AES.new(SK_er, AES.MODE_CBC, init_vector_r)
                decrypted_data = aes_cbc_decrypt.decrypt((ike_auth_r[IKEv2_payload_Encrypted].load[8:])[:-12])
            ...
            break
    return (ipsec_spi_i, ipsec_spi_r)

(ipsec_spi_i, ipsec_spi_r) = gen_ike_auth(psk, SK_ai, SK_pi, ike_nonce_r, ike_spi_i, ike_spi_r, traffic_selector_local_ip, traffic_selector_remote_ip)
...
def gen_ipsec_key(ipsec_nonce_i, ipsec_nonce_r):
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        SK_ei_ipsec = sk[:16]
        SK_ai_ipsec = sk[len(SK_ei_ipsec):20+len(SK_ei_ipsec)]
        SK_er_ipsec = sk[len(SK_ei_ipsec+SK_ai_ipsec):16+len(SK_ei_ipsec+SK_ai_ipsec)]
        SK_ar_ipsec = sk[len(SK_ei_ipsec+SK_ai_ipsec+SK_er_ipsec):20+len(SK_ei_ipsec+SK_ai_ipsec+SK_er_ipsec)]
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        ipsec_sad = {'IPV4' : [[src_ip, dst_ip, int(toHex(ipsec_spi_r), 16), 'ESP', 'AES-CBC', SK_ei_ipsec, 'HMAC-SHA1-96', SK_ai_ipsec],
                               [dst_ip, src_ip, int(toHex(ipsec_spi_i), 16), 'ESP', 'AES-CBC', SK_er_ipsec, 'HMAC-SHA1-96', SK_ar_ipsec]
                              ],
                     'IPV6' : [['*','3ffe::1','*','ESP','NULL','','NULL','']
                              ]
                    }
...
def payload_padding(packet_len):
    pad_len = 0
    if (packet_len + 2)%ENCRYPTION_BLOCK_SIZE == 0:            # the other 2 bytes ESP Trailer which continas "Pad Length" and "Next Header", refer the upper "Substructure of Payload Data"
        pad_len = 0
    else:
        pad_len = ((packet_len + 2)/ENCRYPTION_BLOCK_SIZE + 1 )*ENCRYPTION_BLOCK_SIZE - (packet_len + 2)
...
def gen_create_child__sa_for_rekey_ipsec(ipsec_spi_i, SK_ai, SK_ar, SK_ei, SK_er):
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ipsec_payload_transform_encryption = IKEv2_payload_Transform()
        ipsec_payload_transform_encryption.next_payload = 0x03
        ipsec_payload_transform_encryption.transform_type = 0x01   #encryption
        ipsec_payload_transform_encryption.transform_id = 0x0003     #3DES
    elif ENCRYPTION_MODE == "AES-CBC":
        ipsec_payload_transform_encryption = IKEv2_payload_Transform()
        ipsec_payload_transform_encryption.next_payload = 0x03
        ipsec_payload_transform_encryption.length        = 0x0c
        ipsec_payload_transform_encryption.transform_type = 0x01   #encryption
        ipsec_payload_transform_encryption.transform_id = 0x000c     #AES_CBC
        ipsec_payload_transform_encryption.key_length = 128
    ...
    while True:
        ...
        # elif ike_create_child_sa_response[IP].proto == 17 and ike_create_child_sa_response[UDP].sport == 500 and ike_create_child_sa_response.init_SPI == ike_spi_i :
            ...
            if ENCRYPTION_MODE == "3DES-CBC":
                ...
                #the init_vertor of AES_CBC is 16Bytes. So divide the 8bytes from field "load" to field "iv"
                init_vector_r   = ike_create_child_sa_response[IKEv2_payload_Encrypted].iv + ike_create_child_sa_response[IKEv2_payload_Encrypted].load[:8]     
                aes_cbc_decrypt = AES.new(SK_er, AES.MODE_CBC, init_vector_r)
                decrypted_data = aes_cbc_decrypt.decrypt((ike_create_child_sa_response[IKEv2_payload_Encrypted].load[8:])[:-12])
...
def gen_create_child_sa_for_rekey_ike(ya, ike_spi_i, ike_spi_r, SK_ai, SK_ei):
    ...
    if ENCRYPTION_MODE == "3DES-CBC":
        ...
        ikev2_payload_transform_encryption.length        = 0x0c
        ikev2_payload_transform_encryption.transform_type = 0x01   #encryption
        ikev2_payload_transform_encryption.transform_id = 0x000c     #AES_CBC
        ikev2_payload_transform_encryption.key_length = 128
...
save_ipsec_keys(SK_ai_ipsec, SK_ar_ipsec, SK_ei_ipsec, SK_er_ipsec)
# print "line %d: exit\n" %(lineno())
# exit()
...
if test_rekey_ipsec:
    for iter in xrange(1, 2):
        ...
        esp_init_vector = ''.join(RandString(ENCRYPTION_BLOCK_SIZE))
        encrypted_icmp = ether/IP(src = src_ip, dst = dst_ip, id = 0x0000, flags = 0x02)/ESP(spi = int(toHex(ipsec_spi_r), 16), seq = esp_seq, iv = esp_init_vector, nh = 0x04, pad = pad_str, padlen = pad_len)/icmp
...
pad_len = len(pad_str)
if ENCRYPTION_MODE == "3DES-CBC":
    esp_init_vector = ''.join(RandString(8))
elif ENCRYPTION_MODE == "AES-CBC":
    esp_init_vector = ''.join(RandString(16))
encrypted_icmp = ether/IP(src = src_ip, dst = dst_ip, id = 0x0000, flags = 0x02)/ESP(spi = int(toHex(ipsec_spi_r), 16), seq = esp_seq, iv = esp_init_vector, nh = 0x04, pad = pad_str, padlen = pad_len)/icmp
s.send(encrypted_icmp)
