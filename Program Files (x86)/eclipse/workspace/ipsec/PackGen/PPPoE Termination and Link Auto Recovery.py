...
test_pppoe_server_does_not_reply_bts_lcp_config_request = False
test_pppoe_server_send_padt = False
test_pppoe_server_send_padt_with_wrong_mac = False

test_pppoe_server_send_auth_failed = False
test_pppoe_server_does_not_reply_ipcp_config_request = False
...
while True:
	...
	if ppp is None or ppp[Ether].src == local_mac:			#bypass the packet from local
		...
		if ppp[PPP].proto == 0xc021:			#PPP LCP message
			if test_pppoe_server_does_not_reply_bts_lcp_config_request:
				print "exit from line %s" % (lineno())
				exit()
	...
	#			ppp_lcp_reply.show2()
			...
			if ppp[PPP].code in [2]:				#receive CHAP response
				...
				if test_pppoe_server_send_auth_failed:
					ppp_chap_reply[PPP_CHAP].code = 0x04
					ppp_chap_reply[PPP_CHAP].data = "Failed your sister"
			...
			if ppp[PPP_IPCP].code in [1]:				#ppp ipcp request
				...
				if test_pppoe_server_does_not_reply_ipcp_config_request:
					print "exit from line %s" % (lineno())
					exit()
				...
				for ipcp_option in ppp[PPP_IPCP].options:
					if type(ipcp_option) is PPP_IPCP_Option_IPAddress:
						...
						if ipcp_option.data == "0.0.0.0":
							...
							if test_pppoe_server_send_padt:
								padt = Ether(src=local_mac, dst=remote_mac)\
											/PPPoED(code=0xa7, sessionid=pppoe_sessionid)\
											/PPPoED_Tag(type=0x0203, data="Shutting Down")
								s.send(padt)
							...
							if test_pppoe_server_send_padt_with_wrong_mac:
								padt = Ether(src="00:00:00:00:00:01", dst=remote_mac)\
									/PPPoED(code=0xa7, sessionid=pppoe_sessionid)\
									/PPPoED_Tag(type=0x0203, data="Shutting Down")
								s.send(padt)
								print "exit from line %s" % (lineno())
