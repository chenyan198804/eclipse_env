...
test_pppoe_server_no_auth = False
test_ppooe_server_send_unsupported_auth_method = False
	...
	if ppp is None or ppp[Ether].src == local_mac:			#bypass the packet from local
		...
		if ppp[PPP].proto == 0xc021:			#PPP LCP message
			if send_lcp_request_flag:
				...
				if test_pppoe_server_no_auth:
					ppp_lcp_request.options = PPP_LCP_Option(type=0x05, data=magic_number)
					send_chap_challenge_flag = False

				if test_ppooe_server_send_unsupported_auth_method:		#c227 is EAP
					# ppp_lcp_request.options = PPP_LCP_Option(type=0x03, data="\xc2\x27")*297
					# ppp_lcp_request.options = PPP_LCP_Option(type=0x03, data="\xc2\x27")\
						# /PPP_LCP_Option(type=0x03, data="\xc0\x23")
					ppp_lcp_request.options = PPP_LCP_Option(type=0x03, data="\xc0\x23")\
						/PPP_LCP_Option(type=0x03, data="\xc2\x27")
			...
			elif ppp[PPP_LCP].code in [3]:		#PPP LCP NAK
				ppp_lcp_request.options = PPP_LCP_Option(type=0x05, data=magic_number)\
						/PPP_LCP_Option(type=0x03, data="\xc0\x23")
			...
			elif ppp[PPP_LCP].code in [4]:		#PPP LCP Reject
				ppp_lcp_request.options = PPP_LCP_Option(type=0x05, data=magic_number)\
						/PPP_LCP_Option(type=0x03, data="\xc0\x23")
