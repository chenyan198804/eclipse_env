...
test_pppoe_server_does_not_reply_ipcp_request_message = False
test_pppoe_server_does_reject_ipcp_request_message = False
test_pppoe_server_send_ipcp_request_with_illegal_ip = True		#this scenario contains NULL ip(0.0.0.0), 127.0.0.0/8, 255.255.255.255
test_pppoe_server_send_ipcp_nak_with_illegal_ip = False
...
while True:
	...
	#					  /PPP_PAP_Data(data="rp-pppoe")/PPP_PAP_Data(data="rp-pppoe")
			...
			if ppp[PPP_IPCP].code in [1]:				#ppp ipcp request
				...
				if test_pppoe_server_does_reject_ipcp_request_message:
					ppp_ipcp_reply[PPP_IPCP].code = 0x04
					s.send(ppp_ipcp_reply)
					print "exit from line %s" % (lineno())
					exit()

				if test_pppoe_server_send_ipcp_request_with_illegal_ip:
					...
					ppp_ipcp_request[PPP_IPCP].options = PPP_IPCP_Option_IPAddress(data="127.0.0.2")
					s.send(ppp_ipcp_request)
					test_pppoe_server_send_ipcp_request_with_illegal_ip = False
				...
				for ipcp_option in ppp[PPP_IPCP].options:
					if type(ipcp_option) is PPP_IPCP_Option_IPAddress:
						...
						if ipcp_option.data == "0.0.0.0":
							...
							if test_pppoe_server_send_ipcp_nak_with_illegal_ip:
								ppp_ipcp_reply[PPP_IPCP_Option_IPAddress].data = ""		#the value is: 255.255.255.255, 224.0.0.1, 240.0.0.1, or NULL
							s.send(ppp_ipcp_reply)
						else:
							if test_pppoe_server_does_not_reply_ipcp_request_message:
								print "exit from line %s" % (lineno())
								exit()
