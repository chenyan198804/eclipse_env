...
test_pppoe_server_send_pado_with_service_name_error = False
test_pppoe_server_send_pado_with_ac_system_error = False
test_pppoe_server_send_pado_with_generic_error = False
test_pppoe_server_send_pado_pads_with_legal_tag = False
test_pppoe_server_send_pads_with_service_name_error = False
test_pppoe_server_send_pads_with_ac_system_error = False
test_pppoe_server_send_pads_with_generic_error = True
...
#		print bytes(pado).encode('hex')
		...
		if test_pppoe_server_send_pado_with_service_name_error:
			pado /= PPPoED_Tag(type = 0x0201, data = "service name error")
			s.send(pado)
			exit()
		
		if test_pppoe_server_send_pado_with_ac_system_error:
			pado /= PPPoED_Tag(type = 0x0202, data = "ac system error")
			s.send(pado)
			exit()
		
		if test_pppoe_server_send_pado_with_generic_error:
			pado /= PPPoED_Tag(type = 0x0203, data = "generic error")
			s.send(pado)
			exit()
			
		if test_pppoe_server_send_pado_pads_with_legal_tag:
			pado /=  PPPoED_Tag(type = 0x0105, data = "NSN")\
					/PPPoED_Tag(type = 0x0110, data = "8888")\
					/PPPoED_Tag(type = 0x0000)
...
while True:
	...
	if padr is None or padr[Ether].src == local_mac:
		...
		if test_pppoe_server_send_pado_pads_with_legal_tag:
			pads /=  PPPoED_Tag(type = 0x0105, data = "NSN")\
				/PPPoED_Tag(type = 0x0110, data = "8888")\
				/PPPoED_Tag(type = 0x0000)
		
		if test_pppoe_server_send_pads_with_service_name_error:
			pads /= PPPoED_Tag(type = 0x0201, data = "service name error")
			s.send(pads)
			test_pppoe_server_send_pads_with_service_name_error = False
			continue
		
		if test_pppoe_server_send_pads_with_ac_system_error:
			pads /= PPPoED_Tag(type = 0x0202, data = "ac system error")
			s.send(pads)
			test_pppoe_server_send_pads_with_ac_system_error = False
			continue
		
		if test_pppoe_server_send_pads_with_generic_error:
			pads /= PPPoED_Tag(type = 0x0203, data = "generic error")
			s.send(pads)
			test_pppoe_server_send_pads_with_generic_error = False
			continue
