	...
	if ppp is None or ppp[Ether].src == local_mac:			#bypass the packet from local
		...
		if ppp[PPP].proto == 0xc021 and send_lcp_request_flag:
			...
			# for i in PPP_LCP_Options_Types.keys():
				...
				# s.send(ppp_lcp_request)
			ppp_lcp_request.options = PPP_LCP_Option(type=0, data=RandString(8))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=1, data=RandString(2))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=4, data="\xc0\x25")
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=7)
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=8)
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=9, data="\x01")
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=10, data="\x02")
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=11, data=RandString(2))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=12, data=RandString(6))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=13, data=RandString(1))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=15)
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=17, data=RandString(2))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=18)
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=22, data=RandString(2))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=23, data=RandString(2))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=27, data=RandString(2))
			s.send(ppp_lcp_request)
			
			ppp_lcp_request.options = PPP_LCP_Option(type=29)
			s.send(ppp_lcp_request)
