...
test_pppoe_server_send_padi = True

padi = Ether(src=local_mac, dst="ff:ff:ff:ff:ff:ff")\
				/PPPoED(code=0x09)\
...
#		print bytes(pado).encode('hex')
		...
		if test_pppoe_server_send_padi:
			padi[Ether].src = local_mac
			padi[Ether].dst = remote_mac
			s.send(padi)
			exit()
		...
		pado1 = Ether(src="00:00:00:00:00:01", dst=remote_mac)\
				/PPPoED(code=0x07)\
				/PPPoED_Tag(type=0x0101, data = service_name)\
				/PPPoED_Tag(type=0x0102, data = ac_name)\
				/PPPoED_Tag(type=0x0103, data = tag_dict[0x0103])\
