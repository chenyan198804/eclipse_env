...
test_pppoe_server_does_not_reply_ppp_lcp_request = False
test_pppoe_server_reply_ppp_lcp_request_with_diff_options = False
# test_pppoe_server_dose_not_send_ppp_lcp_request = True
...
while True:
	...
	if ppp is None or ppp[Ether].src == local_mac:			#bypass the packet from local
		...
		if ppp[PPP].proto == 0xc021:			#PPP LCP message
			if test_pppoe_server_does_not_reply_ppp_lcp_request:
				print "exit from line %s" % (lineno())
				exit()
			...
			'''send LCP other request in non-opened state'''
	...
	#			print "-----------------"
				...
				if test_pppoe_server_reply_ppp_lcp_request_with_diff_options:
					ppp_lcp_reply = Ether(src=local_mac, dst=remote_mac)\
						/PPPoE(code=0x00, sessionid=pppoe_sessionid)\
						/PPP()\
						/PPP_LCP(code=0x02)
					ppp_lcp_reply.options = PPP_LCP_Option(type=0x05, data=magic_number)
