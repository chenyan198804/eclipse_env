...
test_pppoe_server_pado_with_illegal_session_id = False
test_pppoe_server_pado_with_two_relay_session_id = False
test_pppoe_server_pads_with_session_id_0xffff = True
...
#		print bytes(pado).encode('hex')
		...
		if test_pppoe_server_pado_with_illegal_session_id:
			pado[PPPoED].sessionid = 0xffff
			s.send(pado)
			test_pppoe_server_pado_with_illegal_session_id = False
			continue
		
		if test_pppoe_server_pado_with_two_relay_session_id:
			pado /= PPPoED_Tag(type = 0x0110, data = ''.join(RandString(12)))\
				   /PPPoED_Tag(type = 0x0110, data = ''.join(RandString(12)))
			s.send(pado)
			test_pppoe_server_pado_with_two_relay_session_id = False
			break
...
while True:
	...
	if padr is None or padr[Ether].src == local_mac:
		...
		if test_pppoe_server_pads_with_session_id_0xffff:
			pads[PPPoED].sessionid = 0xffff
			s.send(pads)
			test_pppoe_server_pads_with_session_id_0xffff = False
			continue
